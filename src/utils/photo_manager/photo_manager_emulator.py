...
class PhotoManagerEmulator(BasePhotoManager):
    def __init__(self, temp_storage="temp/photos", permanent_storage="permanent_storage", sample_images_dir="sample_images"):
    ...
    def ensure_sample_images(self):
        ...
        if not os.listdir(self.sample_images_dir):
            logging.warning("No sample images found in %s. Please add some JPEG files to use as samples.", self.sample_images_dir)

    def take_photo_with_camera(self, project, well, camera_num):
    ...
    def take_photos(self, project, well):
        """Simulates capturing photos using both configured cameras."""
