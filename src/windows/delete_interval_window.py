...
class DeleteIntervalWindow(BaseWindow):
    def __init__(self, project, well, interval, app_instance, parent=None):
        ...
        self.ui.question_label.setText(f"Удалить интервал {self.interval.get_full_name()} и сделанные фотографии?")

        self.ui.yes_pushButton.clicked.connect(self.on_yes_clicked)
        self.ui.no_pushButton.clicked.connect(self.goto_interval)
    ...
    def on_yes_clicked(self):
        self.get_database_manager().delete_interval_and_related_photos(self.interval.id)
        path_to_delete = os.path.join(self.project.media_path, self.well.name, self.interval.get_full_name())
        try:
